...
enum Instructions
{
    ...
    // constants:
    OP_LCONS = 0x10, // store a value in a register, e.g.: lcons r0, 0xA2 0x00 0x00 0x00
    ...
    // register operations:
    OP_MOV = 0x30, // copy a value between registers, e.g.: mov r0, r2
    // stack:
    OP_PUSH = 0x40, // push a register onto the stack, e.g.: push r0
    ...
    // memory:
    OP_STOR = 0x60, // copy a value from a register to a heap address, e.g.: stor 0x08 0x00, r0
    ...
    // arithmetic:
    OP_INC = 0x80, // increment the specified register, e.g.: inc r0
    ...
    OP_MOD,        // store division remainder in first reg, e.g.: mod r0, r1
    ...
    // conversions:
    OP_U2I = 0xA0, // convert an unsigned integer to a signed integer, e.g.: u2i r0
    ...
    // strings:
    OP_PRINT = 0xD0, // print an integer stored in a register, e.g.: print r0
    ...
    // Arduino:
    // TODO: pinMode()
    OP_A_DR = 0xE0, // digital read from a pin to a register, e.g.: dr r0, 0x0A
