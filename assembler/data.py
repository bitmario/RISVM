...
class Opcodes(IntEnum):
    ...
    AND = 0x8D         # and two registers and store result in the first one e.g.: and ax bx
    OR = 0x8E          # or two registers and store result in the first one e.g.: or ax bx
    XOR = 0x8F         # xor two registers and store result in the first one e.g.: xor ax bx
    NOT = 0x90         # not a register and store result e.g.: not ax
    # conversions:
    I2F = 0xA0        # convert an integer stored in a register to a float e.g.: i2f ax
    F2I = 0xA1        # convert a float stored in a register to an int e.g.: f2i ax
    # branching:
    ...
    JGZ = 0xB4        # jump if greater than zero e.g.: jgz ax 0x0A 0x00
    JLZ = 0xB5        # jump if less than zero e.g.: jlz ax 0x0A 0x00
    JE = 0xB6         # jump if equal e.g. je ax bx 0x0A 0x00
    JNE = 0xB7        # jump if not equal e.g. jne ax bx 0x1C 0x00
    JG = 0xB8         # jump if greater e.g. jg ax bx 0x1C 0x00
    JGE = 0xB9        # jump if greater or equal e.g. jge ax bx 0x1C 0x00
    JL = 0xBA         # jump if less e.g. jl ax bx 0x1C 0x00
    JLE = 0xBB        # jump if less or equal e.g. jle ax bx 0x1C 0x00
    # strings:
    ...
    PRINTLN = 0xD3      # print a newline e.g.: println
    I2S = 0xD4          # convert an int stored in a register to a string in a specific address e.g.: i2s ax 0x20 0x00
    S2I = 0xD5          # convert a string at the specified address to an int in a register e.g.: s2i 0x20 0x00 ax
    # Arduino:
